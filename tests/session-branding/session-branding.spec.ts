...
describe("session-branding extension", () => {
  ...
  beforeEach(() => {
    tempCwd = join(tmpdir(), `omp-session-branding-test-${Date.now()}`);
  ...
  it("loads and saves color config correctly", async () => {
    ...
    const brandingPath = join(tempCwd, ".omp", "branding.json");
  ...
  it("retains defaults when branding.json is malformed", async () => {
    ...
    const brandingPath = join(tempCwd, ".omp", "branding.json");
    mkdirSync(join(tempCwd, ".omp"), { recursive: true });
  ...
  it("persists names through the OMP session-name API for the name subcommand", async () => {
    const recorded: Recorded = {
      commands: new Map(),
      events: new Map(),
      setSessionName: jest.fn(),
      getSessionName: jest.fn(() => "Current Session"),
    };
    const fakePi = makeFakePi(recorded);
    extension(fakePi);
    ...
    await recorded.commands
      .get("session-branding")!
      .handler("name Release Candidate", ctx);

    expect(recorded.setSessionName).toHaveBeenCalledWith("Release Candidate");
  });
  ...
  it("rejects invalid colors without saving via direct command", async () => {
    ...
    expect(existsSync(join(tempCwd, ".omp", "branding.json"))).toBe(false);
  });
