---
name: commit
description: Use when a user asks to commit, create a git commit, stage and commit changes, or requests a gitmoji commit message for staged changes.
---
...
6. Ask exactly one final in-conversation confirmation with full message + file list.
...
```
Never include a `Co-authored-by: Claude` trailer in any generated or proposed commit message.
...
Ask for one explicit in-conversation confirmation before committing:
...
| Choosing a gitmoji silently when ambiguous.       | Offer likely options and ask the user to choose.                                                                       |
