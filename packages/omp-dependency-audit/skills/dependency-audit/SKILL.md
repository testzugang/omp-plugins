...
description: Use when an agent needs to audit an npm package, GitHub repository, or pending dependency updates for supply-chain malware and risky lifecycle scripts before installation.
---
...
Wenn der Skill ohne weitere Parameter aufgerufen wird (z.B. `/skill:dependency-audit`), muss **immer zuerst** eine explizite Modus-Auswahl über ein verfügbares OMP-Auswahlwerkzeug erfolgen.
...
2. Existiert bei Auswahl "Projekt-Dependencies" keine `package.json`, gib eine klare Rückfrage: Pfad angeben oder auf OMP-Plugins wechseln.
...
**Szenario B: Globale OMP-Plugins**

1. Nutze bevorzugt die mitgelieferten Hilfsskripte statt ad-hoc Bash-Loops:
   - `scripts/omp-check-current-global-versions.sh`
   - `scripts/omp-check-latest-npm-versions.sh`
   - `scripts/omp-check-git-source-updates.sh`
   - optional End-to-End: `scripts/run_omp_dependency_audit.py`
2. Nutze die Config-Resolution für Sicherheitsrichtlinien (Priorität):
   - `--config /path/to/config.json`
   - `~/.omp/dependency-audit.json`
...
4. `trusted_peer_dependency_scopes` und `trusted_peer_dependency_packages` steuern enge Allowlists für Peer-Dependency-Versionranges. Default: `@oh-my-pi/*` ist als Peer-Dependency-Scope vertrauenswürdig; diese Regel gilt **nur** für `peerDependencies`, nicht für `dependencies`, `devDependencies`, `optionalDependencies`, `overrides` oder `resolutions`.
...
10. Generiere einen Report, der angibt, welche OMP-Plugins sicher aktualisiert werden können. **Präsentiere am Ende des Berichts immer einen maßgeschneiderten nativen `omp plugin upgrade`-Vorschlag (siehe Abschnitt "Natives OMP-Pluginmanagement (`omp plugin upgrade`)"), der blockierte/quarantänisierte oder zu frische Pakete explizit auslässt.**
...
- Trusted Peer-Dependency-Allowlist: breite Peer-Ranges für konfigurierte vertrauenswürdige Paketnamen oder Scopes werden als INFO dokumentiert, nicht blockierend bewertet. Default: `@oh-my-pi/*`.
...
- `peerDependencies` mit breiten Ranges (`*`, `^`, `~`, `>`, `<`) sind für vertrauenswürdige Peer-Scopes aus der Config erlaubt und werden als INFO geführt. Default: `@oh-my-pi/*`. Git-/URL-/File-/Alias-Specs bleiben reviewpflichtig.
...
## Natives OMP-Pluginmanagement (`omp plugin upgrade`)

OMP verwaltet Plugins über `omp plugin install`, `omp plugin link` und `omp plugin upgrade`. Skills eines installierten Plugin-Pakets werden aus dessen `skills/`-Verzeichnis entdeckt. OMP-spezifische Benutzer- und Projektpfade liegen unter `~/.omp` beziehungsweise `.omp`.
...
2. **Spezifischer Update-Vorschlag (Chaining)**: Wenn einzelne Pakete aufgrund von Sicherheitsbedenken oder Altersschwellen ausgelassen werden müssen, generiere einen verketteten Einzelupdate-Befehl mittels `&& \`, um nur die verifizierten Plugins gezielt zu aktualisieren:
   ```bash
   omp plugin upgrade @testzugang/omp-commit && \
   omp plugin upgrade @testzugang/omp-dependency-audit
   ```
...
Um den standardmäßigen `omp plugin upgrade` Befehl im Terminal abzufangen, sodass er automatisch diesen interaktiven Sicherheits-Audit triggert und eine interaktive Auswahl anbietet, kann folgende Shell-Funktion in die Shell-Konfiguration (z. B. `~/.zshrc` oder `~/.bashrc`) eingetragen werden:
...
Nach dem Neuladen der Shell (`source ~/.zshrc`) führt jede Eingabe von `omp plugin upgrade` direkt zu dem interaktiven Audit-Menü.
...
Automatisierte OMP-Plugin-Audits schreiben standardmäßig:

- JSON: `/tmp/omp_audit_aggregated.json`
- Markdown: `/tmp/omp_audit_report.md`
