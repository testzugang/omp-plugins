...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize aggregated OMP dependency audit JSON as Markdown.")
    parser.add_argument("--input", default="/tmp/omp_audit_aggregated.json", help="Input JSON from run_omp_dependency_audit.py")
    parser.add_argument("--output", default="/tmp/omp_audit_report.md", help="Output markdown report path")
...
def append_finding_context(md: list[str], finding: dict[str, Any], indent: str) -> None:
    classification = finding.get("classification")
    if classification:
        md.append(f"{indent}- Classification: `{classification}`")

    if str(finding.get("severity", "")).upper() not in {"HIGH", "CRITICAL"}:
        return
    diff_evidence = finding.get("diff_evidence")
    if not isinstance(diff_evidence, dict):
        return
    for label, key in (("Baseline", "baseline"), ("Candidate", "candidate")):
        evidence_source = diff_evidence.get(key)
        if not isinstance(evidence_source, dict):
            continue
        evidence = evidence_source.get("evidence")
        if evidence:
            md.append(f"{indent}- {label} evidence: `{evidence}`")


def render_markdown(results: list[dict[str, Any]]) -> str:
    md: list[str] = []
    md.append("# 🛡️ Global OMP Dependency Security Audit Report")
    md.append("")
            ...
            if not findings:
                ...
                if first_party_significant:
                    ...
                    for finding in first_party_significant[:12]:
                        ...
                        append_finding_context(md, finding, "    ")
    ...
    safe = [i for i in updates if str(i.get("decision")) == "PASS_WITH_CAUTION"]
    ...
    def get_omp_source(item: dict[str, Any]) -> str:
        return str(item.get("upgrade_target") or item.get("name") or "")

    allowed_updates = safe
    ...
    held_back = review + blocked + deferred + errors
    if held_back:
    ...
    if allowed_updates:
        ...
        has_held_back = bool(review or blocked or deferred or errors)
        requires_qualified_commands = any(
            get_omp_source(item) != str(item.get("name") or "") for item in allowed_updates
        )

        if has_held_back or requires_qualified_commands:
            if has_held_back:
                md.append("Since some updates need manual review, are blocked, quarantined, or too fresh, run the following selective updates:")
            else:
                md.append("Marketplace-qualified updates must retain their marketplace target:")
            md.append("```bash")
            cmd_lines = []
            for item in allowed_updates:
                cmd_lines.append(f"omp plugin upgrade {get_omp_source(item)}")
            md.append(" && \\\n".join(cmd_lines))
            md.append("```")
        else:
            md.append("Since all pending updates are safe and approved, you can update everything at once:")
            md.append("```bash")
            md.append("omp plugin upgrade")
            md.append("```")
