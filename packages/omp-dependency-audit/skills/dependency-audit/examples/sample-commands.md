...
## 7. Reusable helpers for global OMP plugin checks
...
## 8. End-to-end global OMP plugin update audit (static only)

```bash
python3 scripts/run_omp_dependency_audit.py --output /tmp/omp_audit_aggregated.json
python3 scripts/summarize_omp_dependency_audit.py \
  --input /tmp/omp_audit_aggregated.json \
  --output /tmp/omp_audit_report.md
