...
export default function registerPrFindings(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    parameters: pi.typebox.Type.Object({
      prNumber: pi.typebox.Type.Optional(
        pi.typebox.Type.Number({ description: "PR number" }),
      ),
      repo: pi.typebox.Type.Optional(
        pi.typebox.Type.String({
          description: "Repository in owner/repo format",
        }),
      ),
      unresolved: pi.typebox.Type.Optional(
        pi.typebox.Type.Boolean({ description: "Show unresolved findings only" }),
      ),
      severity: pi.typebox.Type.Optional(
        pi.typebox.Type.Union([
          pi.typebox.Type.Literal("blocker"),
          pi.typebox.Type.Literal("warning"),
          pi.typebox.Type.Literal("nit"),
          pi.typebox.Type.Literal("all"),
        ]),
      ),
      includeStale: pi.typebox.Type.Optional(
        pi.typebox.Type.Boolean({
          description: "Include stale findings on old commits",
        }),
      ),
      mine: pi.typebox.Type.Optional(
        pi.typebox.Type.Boolean({
          description: "Show only findings authored by current gh user",
        }),
      ),
      waitForNextReview: pi.typebox.Type.Optional(
        pi.typebox.Type.Boolean({
          description:
            "Wait for new review activity before reading findings (useful right after a push)",
        }),
      ),
      waitMode: pi.typebox.Type.Optional(
        pi.typebox.Type.Union([
          pi.typebox.Type.Literal("new-review-activity"),
          pi.typebox.Type.Literal("checks-finished"),
        ]),
      ),
      waitTimeoutSec: pi.typebox.Type.Optional(
        pi.typebox.Type.Number({
          description: "Timeout for waiting mode in seconds (default 60)",
        }),
      ),
      waitPollSec: pi.typebox.Type.Optional(
        pi.typebox.Type.Number({
          description:
