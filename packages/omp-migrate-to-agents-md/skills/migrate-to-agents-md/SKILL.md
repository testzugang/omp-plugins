...
description: Use when an agent needs to migrate, split, or restructure agent-specific instructions from CLAUDE.md into AGENTS.md while preserving project documentation.
---
...
6. Ask the user what to do with an explicit in-conversation choice.
...
## Claude Code to OMP Cleanup

When moving instructions intended for OMP or generic agents, flag stale Claude Code mechanics instead of copying them blindly:
...
- `Skill` tool instructions → use OMP skill invocation via `/skill:<name>` or OMP skill discovery when applicable
...
| Copying Claude Code command syntax into OMP-facing docs. | Convert or flag it during migration.                                     |
