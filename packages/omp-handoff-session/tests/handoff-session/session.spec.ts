...
describe("Session & File Persistence", () => {
  ...
  it("saves handoff file under target subdirectory, handles slashes via slugify, and resolves collisions safely", async () => {
    ...
    expect(
      saved1.startsWith(path.join(tempDir, "docs", "omp", "handoffs")),
    ).toBe(true);
  ...
  it("rejects path containment violation when trying to escape docs/omp/handoffs using relative breaks", async () => {
