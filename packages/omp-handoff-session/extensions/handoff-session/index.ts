...
function serializeLlmMessages(messages: Message[]): string {
...
async function suggestInitialHandoffOptions(
  ...
  try {
    ...
    const conversationText = serializeLlmMessages(llmMessages).slice(-12000);
    ...
    const response = await complete(
      ...
      {
        systemPrompt:
          "You generate compact JSON defaults for OMP handoff sessions.",
  ...
  pi.registerCommand("handoff-session", {
    ...
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      try {
        ...
        // OMP does not currently expose session-only model scope to extensions, so settings-backed
